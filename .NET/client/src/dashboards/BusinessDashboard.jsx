...
const BusinessDashboard = () => {
  ...
  const handlePayBill = async (id) => {
    try {
      console.log(`Initiating payment for ID (Payment or Pickup): ${id}`);

      // Call pay-bill endpoint which now accepts either paymentId or pickupId
      const response = await paymentAPI.payBill(id);
      console.log('Payment response from backend:', response.data);
      const { orderId, amount, currency, keyId, debugAmount } = response.data;

      console.log('Payment order created:', {
        orderId,
        amountInPaise: amount,
        amountInInr: amount / 100,
        dbAmount: debugAmount
      });
      ...
      // Open Razorpay with the correct amount from backend
      const options = {
        key: keyId,
        amount: amount, // Already in paise from backend
        currency: currency,
        name: 'BinToBloom',
        description: 'Waste Pickup Service Payment',
        order_id: orderId,
        handler: async (razorpayResponse) => {
          try {
            console.log('Payment success, verifying...', razorpayResponse);
      ...
      const errorMsg = errorData?.message || (typeof errorData === 'string' ? errorData : 'Failed to initiate payment');
...
Generated: ${new Date().toLocaleDateString()}`;
  ...
  const renderBilling = () => {
    // Helper to safest check for pending status
    ...
    // Use paymentStatus properly (backend returns paymentStatus, not status)
    const pendingPayments = payments.filter(p => isPending(p.paymentStatus || p.status));
    const historyPayments = payments.filter(p => !isPending(p.paymentStatus || p.status));
                ...
                {pendingPayments.map(payment => (
                  <div key={payment.paymentId} className="payment-item">
                    <div className="payment-details">
                      ...
                      <p>Bill for Pickup #{payment.pickupRequestId || payment.businessId || 'N/A'}</p>
                      <p>{new Date(payment.paymentDate || payment.createdAt || Date.now()).toLocaleDateString()}</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <span className={`payment-status status-${(payment.paymentStatus || payment.status || 'pending').toLowerCase()}`}>
                        {payment.paymentStatus || payment.status}
                      </span>
                      <button
                        className="btn btn-primary"
                        onClick={() => handlePayBill(payment.paymentId)}
              ...
              {historyPayments.length === 0 ? (
                ...
                historyPayments.map(payment => (
                  <div key={payment.paymentId} className="payment-item">
                    ...
                    <span className={`payment-status status-${(payment.paymentStatus || payment.status || 'completed').toLowerCase()}`}>
                      {payment.paymentStatus || payment.status}
                    </span>
                  </div>
                ))
