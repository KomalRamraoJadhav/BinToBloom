          ...
          myRequests.map(request => (
            <div key={request.pickupId} className="request-card">
              <div className="request-header">
                <h3>{request.wasteType}</h3>
                <span className={`status status-${request.pickupStatus.toLowerCase()}`}>
                  {request.pickupStatus === 'PAYMENT_PENDING' ? '⏳ Waiting for Payment' : request.pickupStatus.replace(/_/g, ' ')}
                </span>
              </div>
              <div className="request-details">
                <p><strong>User:</strong> {request.user?.name}</p>
              ...
              <div className="request-actions">
                  ...
                  <>
                    ...
                    {request.user?.role === 'BUSINESS' && (
                      request.paymentStatus ? (
                        <span className="pending-badge" style={{ backgroundColor: '#f59e0b', color: 'white', display: 'flex', alignItems: 'center', gap: '5px' }}>
                          <Clock size={16} />
                ...
                {(request.pickupStatus === 'PAID' || request.pickupStatus === 'IN_PROGRESS' || (request.pickupStatus === 'ASSIGNED' && request.paymentStatus === 'COMPLETED')) && (
                  <button
                    className="btn btn-primary"
                    onClick={() => {
                      setSelectedRequest(request);
