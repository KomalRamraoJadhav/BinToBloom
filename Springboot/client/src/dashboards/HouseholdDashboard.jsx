...
const HouseholdDashboard = () => {
  ...
  // Calculate rank when leaderboard or user changes
  useEffect(() => {
    if (user && leaderboard.length > 0) {
      const myRank = leaderboard.findIndex(item => item.user?.email === user.email) + 1;
      setStats(prev => ({ ...prev, rank: myRank > 0 ? myRank : 'N/A' }));
    }
  }, [user, leaderboard]);
          ...
          <div className="leaderboard">
            {leaderboard.slice(0, 10).map((entry, index) => (
              <div key={entry.householdId} className="leaderboard-item">
                ...
                <span className="name">{entry.user?.name || 'Anonymous'}</span>
